...
Field::Field(glm::vec4 space, std::uint8_t lanes , double ratio ) {
    float x = space.x/((lanes + 2) * ratio + lanes);  //z / ratio;
    float z = x * ratio;

    for (int i = 0; i < lanes ; i++ ) {
        auto meh = i* (x+z);
        this->lanes.emplace_back(std::make_shared<Lane>(z + (x+z) * i, space, x ));
    }
    std::deque<std::shared_ptr<Player>> temp;
    auto test = std::make_shared<Player>(this->lanes[0]->GetLaneChunks()[0]->GetPos());//, this->lanes[3]->GetLaneChunks()[0]->size * 0.75);
    //test->SetCenterPos(this->lanes[4]->GetLaneChunks()[0]->GetCenterPos());
    this->user = test;
    SetOnLane(test, lanes/2);
    auto ent = *test.get();
