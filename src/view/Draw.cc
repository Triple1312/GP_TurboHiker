...
//
//    for ( auto e : entities) {
//        //auto rot_x = glm::rotate(glm::mat4(), glm::radians(this->angle),
//        glm::vec3(1, 0, 0)); auto view = glm::lookAt(glm::vec3(5, 3, 0),
//        glm::vec3(5, 0, 10), glm::vec3(0, 1, 0)); auto proj =
//        glm::perspective(glm::radians(45.f), 800.f/600.f, 0.1f, 0.2f);
//        //auto point_on_screen = proj * view * glm::vec4(e->pos.x, e->pos.y,
//        e->pos.z, 1 );
//        //auto size_on_screen = proj * view * glm::vec4(e->size.x, e->size.y,
//        e->size.z, 0 ); std::deque<glm::vec4> ps = e->GetFace(1);
//
//        sf::ConvexShape sh(4);
...
//
//
//        //sf::RectangleShape rect(sf::Vector2f(e->size.x * scaler, e->size.z *
//        scaler));
...
//            //glm::vec3 cameraR = glm::normalize(glm::cross(camera_up,
//            cameraD));
//
...
//
//
//            glm::mat4 view = glm::lookAt(camera_pos, glm::vec3(0, 0, 0),
//            camera_up); auto proj = glm::perspective(glm::radians(45.f),
//            800.f/600.f, 0.1f, 100.0f);
//
//            //sf::RectangleShape rect(sf::Vector2f(c->size.x * scaler,
//            c->size.z * scaler));
...
//            glm::vec4 p3 =  proj * view  * ps[3];
//            sh.setPoint(3, sf::Vector2f(p3.x* scale, p3.y* scale));
////            std::cout << p0.x << " " << p0.y << " " << p0.z << " " << p0.w
///<< std::endl; /            std::cout << p1.x << " " << p1.y << " " << p1.z <<
///" " << p1.w << std::endl; /            std::cout << p2.x << " " << p2.y << "
///" << p2.z << " " << p2.w << std::endl; /            std::cout << p3.x << " "
///<< p3.y << " " << p3.z << " " << p3.w << std::endl;
//
...
Draw::Draw(sf::RenderWindow &window) : window(window) {
  //    this->entity_shader =
  //    std::make_unique<Shader>("recources/entity.shader");
  unsigned int indices[] = {// Bottom
                            3, 2, 0, 2, 1, 0,
                            ...
                            // Left
                            7, 3, 0, 4, 7, 0};
  // this->cube_indeces = std::make_unique<IndexBuffer>(indices, 6 * 6 *
  // sizeof(unsigned int)); // todo kan hier mislopen
}

// std::shared_ptr<sf::RectangleShape> Draw::CalcFace(glm::vec4 p, glm::vec4 s,
// std::uint8_t face_nr) {
//    if ( face_nr == 0 ) {
//        auto shape = std::make_shared<sf::RectangleShape>(sf::Vector2f (s.x,
//        s.z));
//
//    }
//}
