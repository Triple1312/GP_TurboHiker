...
Text::Text(const std::string &font, float x, float y, float scale) :
  ...
  vblo_.Push(4);  // points + coords

  this->ibo_ = std::make_unique<IndexBuffer>();
...
void Text::Draw( ) {
  ...
  for (auto c : this->string_) {
    ...
    vbo_.ChangeBufferData(vertices);
    Renderer::Get()->Draw(*vao_,*ibo_,shader_);
    x += (ch.advance_ >> 6) * scale_;
