...
IndexBuffer::IndexBuffer(
    ...
    unsigned int count)  // todo make unsigned char if needed
    : m_Count(count) {
  // ASSERT(sizeof(unsigned int) == sizeof(GLuint));

  glGenBuffers(1, &m_RendererID);  // Generate a single buffer
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
               m_RendererID);  // Select the buffer to be drawn
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * sizeof(unsigned int), data,
               GL_STATIC_DRAW);  // Add the data to the buffer
}

IndexBuffer::IndexBuffer() {
  m_Count = 36;
  unsigned int data[] = {// Bottom
                         3, 2, 0, 2, 1, 0,
