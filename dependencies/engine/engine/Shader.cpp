...
unsigned int Shader::CompileShader(unsigned int type,
  ...
  const char* src =
      source
          .c_str();  // Return the pointer of the first character of the source
  glShaderSource(id, 1, &src, nullptr);  // Specify the shader source code
  glCompileShader(id);

  // Error handling
  int result;
  glGetShaderiv(id, GL_COMPILE_STATUS,
                &result);  // Returns the compile status parameter
  if (result == GL_FALSE) {
    int length;
    glGetShaderiv(id, GL_INFO_LOG_LENGTH, &length);
    char* message = (char*)alloca(
        length *
        sizeof(char));  // Allocate this on the stack dynamically because 'char
                        // message[length]' is not allowed
...
unsigned int Shader::CreateShader(const std::string& vertexShader,
  ...
  unsigned int program =
      glCreateProgram();  // Create a shader program to attach shader to
